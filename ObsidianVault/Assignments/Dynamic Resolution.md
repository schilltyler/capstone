...
Implement helper functions to print detailed information about ELF binaries.

1. **Implement Debug Functions:**
...
## Part 2: Dynamic Symbol Resolution (intended)

In this part you will use intended dynamic loading functions to resolve symbols at runtime and call functions. In particular, your program must find the runtime base address of `libc.so.6`, and then find the  virtual address of `printf`.
...
## Part 3: Dynamic Symbol Resolution (Hardmode)

In this part, you will implement the same functionality as part 2—but **without** using any dynamic loading functions (no `dlopen`, `dlsym`, ...etc).

### 3.1: Find the Base Address Using `/proc/self/maps`
...
### 3.2: Parse the ELF Structure to Find `printf`

1. Review the ELF header, program headers, and dynamic segment entries (such as `DT_STRTAB` and `DT_SYMTAB`).  
        _Hint:_ Check `man elf` for details.
2. **Locate the Dynamic Segment:**  
    - Starting at the base address from Step 3.1 access the ELF header and then iterate over the program headers.
    - Identify the program header that describes the dynamic segment (`PT_DYNAMIC`).
3. **Extract the Symbol and String Tables:**
...
Re-implement the dynamic resolution from Part 4 **using only direct syscalls** (such as `mmap`, `openat`, `read`, etc.), avoiding any libc functions (or any library for that matter!). note your binary should be `static`
